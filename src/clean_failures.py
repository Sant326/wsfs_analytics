...
# 1. Leer CSV
df = pd.read_csv('../data/failures.csv')
...
columnas_texto = ['failure_category', 'severity', 'description']
df_failures_limpio[columnas_texto] = df_failures_limpio[columnas_texto].apply(normalizar_espacios)


#quitar filas duplicadas en todo el dataset
df_failures_limpio = df_failures_limpio.drop_duplicates(keep='last',inplace=False)

print("df_failures_limpio sin duplciados: ")
print(df_failures_limpio)
print(df_failures_limpio.info())
...
columnas = ['failure_category', 'severity', 'description']
df_failures_limpio[columnas] = df_failures_limpio[columnas].apply(quitar_acentos_mantener_ñ)
print("df_final: ")
print(df_failures_limpio)

df_final = df_failures_limpio
...
##engine = create_engine('postgresql://myuser:mypassword@localhost:5433p/maquinaria_agroforestal')


# restriccion id en equipment debe existir para poder cargar en failures
df_equipment_dbfailures = pd.read_sql("select equipment_id from equipment", engine)
ids_validosenequipment = df_equipment_dbfailures['equipment_id'].unique()
...
print("✅ Datos cargados COMPLETOS en tabla failures!")
