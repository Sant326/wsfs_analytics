...
df_sales_limpio = df.drop_duplicates(keep='last',inplace=False)
# imprime la etiqueta y hace un salto de linea y muestra el dataframe
print(f"df-sales-limpio\n{df_sales_limpio}")
print(df_sales_limpio.info())

# ids validos devuelve un array con los ids  del datframe customer luego compara customer_id y si existe en el datframecustomer-columna customer_id lo copia
#ids_validos = df_customer['customer_id'].unique()
#df_sales_limpio = df_sales_limpio[df_sales_limpio['customer_id'].isin(ids_validos)].copy()

print('df_sales sin customer_id')
...
df_customer_db = pd.read_sql("select customer_id from customer", engine)
#obtenemos customer_id
ids_validos = df_customer_db['customer_id'].unique()
